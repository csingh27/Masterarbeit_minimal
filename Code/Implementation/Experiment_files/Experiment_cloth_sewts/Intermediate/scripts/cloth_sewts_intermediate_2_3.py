...
class Cloth(base.Task):
  ...
  def get_reward(self, physics):
    ...
    dist1 = np.sqrt((current_pos_G0_0[0] - X_G0_0)**2 + (current_pos_G0_0[1] - Y_G0_0)**2 + (current_pos_G0_0[2])**2) 
    dist2 = np.sqrt((current_pos_G0_1[0] - X_G0_1)**2 + (current_pos_G0_1[1] - Y_G0_1)**2 + (current_pos_G0_1[2])**2) 
    dist3 = np.sqrt((current_pos_G0_2[0] - X_G0_2)**2 + (current_pos_G0_2[1] - Y_G0_2)**2 +  (current_pos_G0_2[2])**2)     
    # reward = -1 * (dist1 + dist2 + dist3)

    # distance between points # to avoid points on top of each other
    dist_G00_G01 = np.sqrt((current_pos_G0_0[0] - current_pos_G0_1[0])**2 + (current_pos_G0_0[1] - current_pos_G0_1[1])**2)
    dist_G01_G02 = np.sqrt((current_pos_G0_1[0] - current_pos_G0_2[0])**2 + (current_pos_G0_1[1] - current_pos_G0_2[1])**2)
    dist_G00_G02 = np.sqrt((current_pos_G0_0[0] - current_pos_G0_2[0])**2 + (current_pos_G0_0[1] - current_pos_G0_2[1])**2)


    if dist_G00_G01 < 0.02:
        punishment = -10000 * dist_G00_G01

    if dist_G01_G02 < 0.02:
        punishment += -10000 * dist_G00_G01
    
    if dist_G00_G02 < 0.04:
        punishment += -10000 * dist_G00_G01
    ...
    punishment += -10000 * (current_pos_G1_0[2] + current_pos_G1_2[2] + current_pos_G1_2[2]) # Keeping the neighbouring particles on the ground as well

    # checking if the other seam is perpendicular to our seam or not

    current_pos_G2_0 = physics.named.data.geom_xpos['G2_0']
 
    dist_G00_G10 = np.sqrt((current_pos_G0_0[0] - current_pos_G1_0[0])**2 + (current_pos_G1_0[1] - current_pos_G0_0[1])**2)
    dist_G00_G20 = np.sqrt((current_pos_G0_0[0] - current_pos_G2_0[0])**2 + (current_pos_G2_0[1] - current_pos_G0_0[1])**2)
    dist_G20_G02 = np.sqrt((current_pos_G2_0[0] - current_pos_G0_2[0])**2 + (current_pos_G2_0[1] - current_pos_G0_2[1])**2)
    dist_G10_G01 = np.sqrt((current_pos_G1_0[0] - current_pos_G0_1[0])**2 + (current_pos_G1_0[1] - current_pos_G0_1[1])**2)


    pythogores_triangle1 = dist_G01_G10**2 - dist_G00_G01**2 - dist_G10_00**2
    pythogores_triangle2 = dist_G02_G20**2 - dist_G00_G02**2 - dist_G20_00**2

    if pythagores_triangle1 > 0.01:
        punishment += -200

    if pythagores_triangle2 > 0.01:
        punishment += -200

    reward = reward1 + reward2 + reward3 + punishment
